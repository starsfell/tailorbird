...
# 直接查 SQLite
sqlite3 data/birdye.db "SELECT rating, COUNT(*) FROM photos WHERE deleted_at IS NULL GROUP BY rating;"
```

**数据库迁移**:`schema.py` 的 `init_db()` 在启动时自动 `ALTER TABLE` 加新列。旧数据库兼容,无需手动迁移。

**清除 AI 结果重算**:删 `data/birdye.db` 重新扫描即可。缩略图缓存 `data/thumbs/` 可保留(scanner 会自动跳过已存在的)。
...
| 现象 | 原因 / 处理 |
|---|---|
| 启动报「找不到 birdye conda 环境」 | 先 `conda create -n birdye python=3.11`,然后 `pip install -r backend/requirements.txt` |
