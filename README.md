...
| 视图 | 用途 |
|---|---|
| **网格视图** | 按连拍组分块显示,组内最佳绿框高亮,组首带「全选本组 / 选中本组非最佳」两个快捷按钮 |
...
两套并行,方便左撇子(左手鼠标 + 右手键盘):**左手键**沿用习惯,**右手键**集中在键盘右侧 Home 行。两套始终都生效;底栏 / 侧栏的 `⌨ 左手键 ⇄ 右手键` 按钮只切换界面上提示哪一套(记忆在 localStorage)。

| 动作 | 左手键 | 右手键 |
|---|---|---|
| 选中 / 取消选中 | 单击 | 单击 |
| 打开详情视图 | 双击 / Space | 双击 / Space |
| 全选当前视图 | A | O |
| 清空选择 / 关闭弹窗 | Esc / M | Esc / M |
| 选中所有「非组内最佳」 | B | N |
| 对比选中的(2–9 张) | C | L |
| 删除选中 | D | J |
| **全选删除**(删当前全部可见,有二次确认) | — | P |
| 堆栈选中的 / 对比视图切换联动 | S | S / K(联动) |
| 给选中的批量加 / 删标签 | T | ; |
| 在 Finder 中显示第一张选中 | R | U |
| 详情页下一张 | → | ; |
| 详情页上一张 | ← / K | L |

> 详情页里 `J` = 删除(与网格一致),所以翻页用 `;`(下)/`L`(上),方向键 `←/→` 仍可用。
